...
{
    public class Tax: Unit, IObserver
    {
        public Tax(TimeSeries series, TaxOptions options,int? illegalMode = null):base(){
            Options = options;
            DataSeries = series;
        ...
        public override void Update(object sender, EventArgs e)
        {
            ...
            if (categories.Contains(Options.CategoryUses))
                rate_per_day = (ts.Balance(TransactionSource.WaterCost) + ts.Balance(TransactionSource.FreeWater) + ts.Balance(TransactionSource.WaterPattern) + ts.Balance(TransactionSource.WasteWater) + ts.Balance(TransactionSource.Seasonal)) / usage.Duration.Days.ToDouble();
            else
                rate_per_day = (ts.Balance(TransactionSource.WaterCost) + ts.Balance(TransactionSource.FreeWater) + ts.Balance(TransactionSource.WasteWater) + ts.Balance(TransactionSource.Subscription) + ts.Balance(TransactionSource.SubscriptionSewage) + ts.Balance(TransactionSource.Seasonal) + ts.Balance(TransactionSource.Made7)) / usage.Duration.Days.ToDouble();
            ...
            Effect(new Transaction(TransactionSource.Tax,TransactionType.Debit,price));
            if (categories.Contains(Options.CategoryUses))
                Effect(new Transaction(TransactionSource.Tax,TransactionType.Discount,price));
        }
